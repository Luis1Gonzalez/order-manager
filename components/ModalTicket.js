...
import CompTicketUp from "./CompTicketUp";
...
const ModalTicket = () => {
  ...
  const [theLast, setTheLast] = useState([]);
  ...
  useEffect(() => {
    const obtaingTheLast = () => {
      const theLlastOrder = orders[orders.length - 1];
      setTheLast(theLlastOrder);

      const theLastOrderDetails = orders[orders.length - 1]?.createdOrder;
      setAllDetails(theLastOrderDetails);
    };
    obtaingTheLast();
  }, [orders, setIsLoading, theLast]);
            ...
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full sm:p-6">
              ...
              <div className="sm:flex sm:items-start">
                <div className="mt-3 text-center sm:mt-0 sm:text-left w-full flex flex-col gap-7 items-center">
                  {!seeTicket && <CompTicketUp />}
                  {seeTicket && (
                    <CompTicket theLast={theLast} allDetails={allDetails} />
                  )}
                </div>
              </div>
            </div>
